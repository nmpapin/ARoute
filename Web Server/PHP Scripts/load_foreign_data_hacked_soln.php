<?php
	...
	for($i = 96; $i < 97; $i++)
	{	
		$headsigns = json_decode(do_post_request("http://spice.ridecell.com/desktop/route_headsigns/", array("route_id" => $i)), true);
		$headsigns = $headsigns[1];
		$headsign_route_map[$i] = $headsigns;
		
		foreach($headsigns AS $val2)
		{
			$head_val = "(".mysql_real_escape_string($val2['direction']).",'".mysql_real_escape_string($val2['headsign'])."',".mysql_real_escape_string($i).")";
